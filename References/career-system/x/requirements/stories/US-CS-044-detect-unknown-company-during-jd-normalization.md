...
As Paul, I want the JD normalizer to detect unknown companies so that bad normalized filenames and metadata are not silently created.
...
```bash
grep -R "US-CS-044" docs/requirements || true
```
